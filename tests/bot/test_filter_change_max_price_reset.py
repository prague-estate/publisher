...
async def test_filter_change_max_price_reset_happy_path(payload, expected_state):
    ...
    update_user_settings(query_mock.from_user.id, max_price=payload)

    await publisher.handlers.filter_price.filter_change_max_price_reset(query_mock)

    assert get_user_settings(query_mock.from_user.id).max_price is expected_state
