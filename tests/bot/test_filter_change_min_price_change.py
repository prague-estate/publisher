...
async def test_filter_change_min_price_change_happy_path():
    query_mock = AsyncMock()
    query_mock.from_user.id = 1
    state_mock = AsyncMock()

    await publisher.handlers.filter_price.filter_change_min_price_change(query_mock, state_mock)
...
async def test_filter_change_min_price_change_process_happy_path(payload: str, success: bool):
    ...
    await publisher.handlers.filter_price.filter_change_min_price_change_process(message_mock, state_mock)

    if success:
        state_mock.clear.assert_called_once()
        message_mock.answer.assert_called_once()
        assert get_user_settings(user_id=1).min_price == int(payload)
